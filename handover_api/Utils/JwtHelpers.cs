...
namespace handover_api.Utils
{
    public class JwtHelpers
    {
        ...
        public string GenerateToken(string userName,short authValue, PermissionSetting permissionSetting, int expireMinutes = 30)
        {
            ...
            claims.Add(new Claim(ClaimTypes.Role, authValue.ToString()));
            claims.Add(new Claim("permissions", JsonSerializer.Serialize(permissionSetting)));
        ...
        public AuthValueAndPermissionSetting? GetPermissionSetting(ClaimsPrincipal? user)
        {
            ...
            var permissionSetting = JsonSerializer.Deserialize<PermissionSetting>(permissionSettingString);
            var roleClaim = user.FindFirstValue(ClaimTypes.Role);
            if(roleClaim == null) return null;
            short authValue = Convert.ToInt16(roleClaim);
            var authValueAndPermissionSetting = new AuthValueAndPermissionSetting(authValue, permissionSetting);

            return authValueAndPermissionSetting;
        }
    }
}
